<template>
  <div>
    <!-- Hero Section con Resplandor de los colores del Logo -->
    <section class="relative max-w-[1400px] mx-auto px-4 py-20 lg:py-28 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
      ...
      <div class="space-y-8 relative z-10">
        ...
        <div class="flex gap-4 pt-2">
          <button class="bg-zinc-100 hover:bg-white text-zinc-950 px-8 py-3 rounded font-black text-sm uppercase tracking-wide transition-colors shadow-lg shadow-white/5">
            Ver Programación
          </button>
        </div>
      </div>
      ...
      <div class="bg-zinc-900/50 backdrop-blur-xl rounded-2xl p-8 border border-zinc-800/80 shadow-[0_0_40px_rgba(0,0,0,0.8)] relative">
        ...
        <div v-else-if="clubes && clubes.length > 0" class="grid grid-cols-3 sm:grid-cols-4 gap-4 relative z-10">
          <div v-for="club in clubes" :key="club.id" class="flex flex-col items-center justify-center group cursor-pointer p-2 rounded-xl hover:bg-zinc-800/50 transition-all">
            <div class="w-16 h-16 bg-gradient-to-br from-zinc-800 to-zinc-900 rounded-full flex items-center justify-center border border-zinc-700 shadow-inner group-hover:border-amber-600 transition-all mb-3">
              <span class="text-xl font-black text-zinc-300">{{ club.nombre.charAt(0) }}</span>
            </div>
    ...
    <!-- Transmisión En Vivo (Aparece solo si hay un youtube_live_id) -->
      ...
      <div class="bg-zinc-950 border border-zinc-800 rounded-2xl p-2 sm:p-4 shadow-[0_0_50px_rgba(0,0,0,0.9)] relative overflow-hidden">
        ...
        <div class="relative w-full overflow-hidden rounded-xl bg-black border border-zinc-900" style="padding-top: 56.25%;">
          <iframe 
            ...
            :src="`https://www.youtube.com/embed/${partidoEnVivo.youtube_live_id}?autoplay=0&rel=0`" 
    ...
    <!-- Match Center (3 Columnas Dark Mode) -->
    <section class="max-w-[1400px] mx-auto px-4 py-16">
      <div class="flex gap-3 mb-8 border-b border-zinc-800 pb-4 overflow-x-auto">
        <button class="bg-zinc-800 text-zinc-100 px-6 py-2 rounded text-xs font-black uppercase tracking-wider border-l-2 border-amber-500 shadow-md">Todo Competidor</button>
        <button class="bg-transparent hover:bg-zinc-900 text-zinc-500 hover:text-zinc-300 px-6 py-2 rounded text-xs font-black uppercase tracking-wider transition-colors">U17</button>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        <!-- Tabla de Posiciones Dark -->
        <div class="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-6 shadow-xl backdrop-blur-sm">
          ...
          <table class="w-full text-sm">
            <thead class="text-[10px] text-zinc-500 uppercase tracking-widest border-b border-zinc-800">
              <tr>
                <th class="text-left font-black pb-3">POS</th>
                <th class="text-left font-black pb-3">EQUIPO</th>
                <th class="text-center font-black pb-3">PJ</th>
                <th class="text-center font-black pb-3 text-amber-500">PTS</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-zinc-800/50">
              <tr class="hover:bg-zinc-800/30 transition-colors group">
                <td class="py-4 font-black text-amber-500">1</td>
                <td class="py-4 font-bold flex items-center gap-3">
                  <div class="w-7 h-7 bg-zinc-800 border border-zinc-700 rounded-full group-hover:border-amber-500 transition-colors"></div> 
                  <span class="text-zinc-200">Universitarios</span>
                </td>
                <td class="py-4 text-center text-zinc-500 font-medium">5</td>
                <td class="py-4 text-center font-black text-zinc-100">10</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-6 shadow-xl backdrop-blur-sm">
          ...
          <div class="space-y-4">
            <div class="bg-zinc-950/50 border border-zinc-800 rounded-lg p-5 relative overflow-hidden group hover:border-zinc-700 transition-colors">
              <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-red-800 via-zinc-800 to-blue-700 opacity-50"></div>
              <div class="text-[10px] text-zinc-500 font-black tracking-widest text-center uppercase mb-4">Finalizado • 19 Jul 2026</div>
              <div class="flex justify-between items-center mb-1">
                <div class="flex flex-col items-center gap-2 w-1/3">
                  <div class="w-10 h-10 bg-zinc-800 border-2 border-red-900/50 rounded-full"></div>
                  <span class="font-bold text-xs text-zinc-300">UNI</span>
                </div>
                <div class="flex flex-col items-center w-1/3">
                  <div class="text-3xl font-black tracking-tighter text-zinc-100">71 <span class="text-zinc-700">-</span> 61</div>
                </div>
                <div class="flex flex-col items-center gap-2 w-1/3">
                  <div class="w-10 h-10 bg-zinc-800 border-2 border-blue-800/50 rounded-full"></div>
                  <span class="font-bold text-xs text-zinc-300">CBA</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div v-for="partido in proximosPartidos" :key="partido.id" class="bg-zinc-950/50 border border-zinc-800 rounded-lg p-5 hover:border-zinc-700 transition-colors">
              <div class="flex justify-between items-center text-[10px] font-black uppercase tracking-widest mb-4">
                <span class="text-amber-500 flex items-center gap-1">
                  ...
                  {{ new Date(partido.fecha_hora).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' }) }} HRS
                </span>
              ...
              <div class="flex justify-between items-center">
                <div class="flex items-center gap-3 w-2/5">
                  <div class="w-8 h-8 bg-zinc-800 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-zinc-500">{{ partido.local.nombre.charAt(0) }}</div>
                ...
                <div class="flex items-center gap-3 w-2/5 justify-end">
                  ...
                  <div class="w-8 h-8 bg-zinc-800 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-zinc-500">{{ partido.visita.nombre.charAt(0) }}</div>
                </div>
              </div>
            </div>
          </div>
...
const { data: clubes, pending } = useFetch('/api/clubes')
